...
public class MainForm : Form
{
    ...
    // Test Mode UI
    private readonly Button _testModeButton = new() { Text = "Test Effects", AutoSize = true };
    ...
    {
        var testPanel = new FlowLayoutPanel { FlowDirection = FlowDirection.LeftToRight, AutoSize = true };
    ...
    private void SwitchToNextWindow()
    {
        ...
        try
        {
            ...
            var nonBlacklistedWindows = validWindows.Where(window =>
            {
            ...
            if (nonBlacklistedWindows.Count > 0)
            {
                validWindows = nonBlacklistedWindows;
                Debug.WriteLine($"After blacklist filter: {validWindows.Count} windows available");
            }
            else
            {
                Debug.WriteLine("All games blacklisted, using original list");
            }
